...
contract PrivateSpaceCrowdsale is Crowdsale, TimedCrowdsale {

    //Trackeado de contribuciones de Investors
    uint256 public investorMinCap = 35000000000000000; //0.035 ETH
    uint256 public investorHardCap = 20000000000000000000; //20 ETH
    ...
    /**
    * @dev Returns de amount contributed so far by an specific user
    ...
    */
    ...
    function setCrowdSaleStage(uint _stage) public onlyAdmin {
      if (uint(CrowdsaleStage.PreICO) == _stage) {
          stage = CrowdsaleStage.PreICO;
      } else if (uint(CrowdsaleStage.ICO) == _stage) {
          stage = CrowdsaleStage.ICO;
      } else if (uint(CrowdsaleStage.PostICO) == _stage) {
          stage = CrowdsaleStagePostICO;
      }
    
      if(stage == CrowdStage.PreICO) {
        rate =100000;
      } else if (stage == CrowdStage.ICO)   {
        rate = 75000;  
      } else if (stage == CrowdStage.PostICO) {
          rate = 50000;
      }
    }
  ...
  */
  function finalization() internal {
    if(goalReached()) {
     // MintableToken _mintableToken = MintableToken(token); Todo Minted/ sale 1.000.000 from 1.000.000.000
